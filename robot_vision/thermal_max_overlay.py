...
DETECT_INTERVAL_FRAMES = 6   # 서버 이전 후 출력 30fps 기준, 검출 주기 ~5Hz(200ms)
...
# ------------------------------------------------------------------
class RTSPGrabber:
    """RTSP 최신 프레임만 유지. 연결이 끊기면(로봇 off 등) 백그라운드 스레드에서
    자동 재연결한다 — 프로세스는 종료되지 않고 카메라 복귀 시 스스로 복구한다.
    최초 연결 실패와 스트림 중간 끊김을 같은 메커니즘으로 처리한다."""

    _MAX_READ_FAILS = 3       # 연속 read 실패가 이 횟수를 넘으면 끊긴 것으로 보고 재연결
    _RECONNECT_DELAY = 3.0    # 재연결 시도 간격(초)
    _IO_TIMEOUT_MS = 5000     # FFmpeg open/read 타임아웃(ms). 기본 30s는 끊김 감지가 너무 느림
    ...
    def __init__(self, url):
        ...
        self.lock = threading.Lock()
        self._frame = None
        self._seq = 0          # 새 원본 프레임 저장 시마다 증가 — 소비측 중복 처리 스킵용
        self._running = False
        self._thread = None

    def _open(self):
        # FFmpeg 기본 open/read 타임아웃이 30초라, 스트림이 죽으면 read()가 30초씩
        # 블로킹되어 끊김 감지·재연결이 매우 느려진다. 타임아웃을 5초로 낮춰
        # 빠르게 실패→재연결하도록 한다. (OpenCV 4.5.2+ params 생성자)
        params = []
        ...
        if open_to is not None:
            params += [int(open_to), self._IO_TIMEOUT_MS]
        if read_to is not None:
            params += [int(read_to), self._IO_TIMEOUT_MS]
        if params:
    ...
    def _loop(self):
        fails = 0
        while self._running:
            # 연결이 없으면(최초/끊김) 재연결 시도. cap 접근은 이 스레드 단독이므로
            # read()/release()가 cross-thread로 충돌하지 않는다.
            if self.cap is None or not self.cap.isOpened():
                ...
                if cap.isOpened():
                    ...
                    cap.release()
                    print(f"[RTSP] 연결 대기 중 (카메라/로봇 off?) — {self._RECONNECT_DELAY:.0f}s 후 재시도")
    ...
    def read(self):
        """(최신 프레임, 시퀀스 번호)를 반환. seq가 직전과 같으면 원본이 안 바뀐 것."""
        with self.lock:
...
def run_rtsp_overlay(restream_port=8556):
    ...
    # 연결은 grabber 내부 스레드가 자동으로 (재)시도한다. 카메라/로봇이 꺼져 있어도
    # 여기서 종료하지 않고 살아서 대기 → 로봇 복귀 시 스스로 복구한다.
    ...
    def frame_producer():
        ...
        while not stop_event.is_set():
            raw, raw_seq = grabber.read()
            if raw is None:
                time.sleep(0.01)
                continue
            # 원본 프레임이 안 바뀌었으면(카메라 fps < 출력 목표 fps 등) resize/추론/
            # 인코딩을 통째로 스킵 — latest_jpeg는 직전 프레임 유지. 페이싱만 지켜
            # busy-spin 방지. 검출 정확도엔 영향 없음(새 프레임이 없으니 추적할 것도 없음).
